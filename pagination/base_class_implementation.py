...
class BaseImplementation(BasePagination):
    ...
    def get_scrape_url(self):
        return self.pagination_config.get('scrape_url', '')

    def get_single_hit_flag(self):
        return self.pagination_config.get('single_hit_case_flag', False)

    def get_data(self, **kwargs):
        string_data = self.pagination_config.get('data', '')
        return string_data.format(kwargs)

    def get_loop_flag(self):
        return -1 if self.pagination_config.get('pagination').get('yearwise', False) else 1
